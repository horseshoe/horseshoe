...
Objects that will result in foreach sections include arrays, iterables, and streams. All other objects result in conditional sections.
...
| `@File('name': [filename], 'encoding': 'UTF-16', 'overwrite': false, 'append')` | Sends rendered output to the file with the specified name, using the specified encoding. <b>If overwrite is false (or omitted) then the file is only modified when the contents of the file are different than the rendered output.</b> If append is specified as true then the rendered output is appended to the file. |
...
String literals are sequences of characters wrapped in either single quotes or double quotes. When wrapped in single quotes the only escape sequence is `''` (double single quote) to escape a single quote. All other characters in a single-quoted string literal are the exact characters given. The following escape sequences can be used in double-quoted string literals:
